...
const getStatusCode = (error) => http_error_model_1.isHttpError(error)
    ? error.status
    : http_interface_1.HttpStatus.INTERNAL_SERVER_ERROR;
const errorFactory = (status) => (error) => ({
    error: http_error_model_1.isHttpError(error)
        ? { status, message: error.message, data: error.data, context: error.context }
        : { status, message: error.message },
});
exports.defaultError$ = req$ => req$.pipe(operators_1.map(({ error = defaultHttpError }) => {
