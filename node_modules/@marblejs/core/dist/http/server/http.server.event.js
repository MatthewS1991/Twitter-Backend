...
const event_factory_1 = require("../../event/event.factory");
var ServerEventType;
...
exports.ServerEvent = {
    connect: event_factory_1.createEvent(ServerEventType.CONNECT, (request, socket, head) => ({ request, socket, head })),
    connection: event_factory_1.createEvent(ServerEventType.CONNECTION, (socket) => ({ socket })),
    clientError: event_factory_1.createEvent(ServerEventType.CLIENT_ERROR, (error, socket) => ({ error, socket })),
    close: event_factory_1.createEvent(ServerEventType.CLOSE),
    checkContinue: event_factory_1.createEvent(ServerEventType.CHECK_CONTINUE, (request, response) => ({ request, response })),
    checkExpectation: event_factory_1.createEvent(ServerEventType.CHECK_EXPECTATION, (request, response) => ({ request, response })),
    error: event_factory_1.createEvent(ServerEventType.ERROR, (error) => ({ error })),
    request: event_factory_1.createEvent(ServerEventType.REQUEST, (request, response) => ({ request, response })),
    upgrade: event_factory_1.createEvent(ServerEventType.UPGRADE, (request, socket, head) => ({ request, socket, head })),
    listening: event_factory_1.createEvent(ServerEventType.LISTENING, (port, host) => ({ port, host })),
};
